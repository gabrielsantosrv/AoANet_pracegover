        ...
        while True:
            if epoch_done:
                ...
                if opt.self_critical_after != -1 and epoch >= opt.self_critical_after:
                    sc_flag = True
                    init_scorer(opt.cached_tokens, opt.cider_alpha, opt.penalize_repetition,
                                coefficient_len=opt.coefficient_len, coefficient_rep=opt.coefficient_rep)

                    curr_cider_max = max(curr_cider)
                    if curr_cider_max > last_cider_max:
                        nonimprovement_epochs = 0
                    else:
                        nonimprovement_epochs += 1

                    if nonimprovement_epochs >= opt.patience_cider:
                        break

                    last_cider_max = curr_cider_max
                    curr_cider = []
                else:
                    sc_flag = False
            ...
            if (iteration % opt.save_checkpoint_every == 0):
                ...
                if sc_flag and 'CIDEr' in lang_stats:
                    curr_cider.append(lang_stats['CIDEr'])
