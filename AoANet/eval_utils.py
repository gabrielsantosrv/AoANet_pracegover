...
#def init_scorer():
    #global BERT_scorer
    #BERT_scorer = BERT_scorer or BERTScore(metric='recall')
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    compute_bert = eval_kwargs.get('compute_bert', False)
    ...
    while True:
        ...
        groundtruth = decode_gts( data['gts'], loader.get_vocab())
        if lang_eval == 1 and compute_bert:
            groundtruth = decode_gts( data['gts'], loader.get_vocab())

            mean_bertscore, _ = BERT_scorer.compute_score(groundtruth, sents)
            bertscore += mean_bertscore

            if verbose:
                print('BERT-Score:', mean_bertscore)
        ...
        if verbose:
            print('evaluating validation preformance... {}/{} ({})'.format(ix0 - 1, ix1, loss))
    ...
    if lang_eval == 1:
        ...
        if compute_bert:
            lang_stats['bert_score'] = bertscore / iterations
